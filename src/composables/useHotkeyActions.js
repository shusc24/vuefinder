...
export function useHotkeyActions(app) {
  ...
  const handleKeyboardShortcuts = (e) => {
    ...
    if (
      e.code === KEYBOARD_SHORTCUTS.F2 &&
      app.features.includes(FEATURES.RENAME)
    ) {
      app.dragSelect.getCount() !== 1 ||
        app.modal.open(ModalRename, { items: app.dragSelect.getSelected() });
    }
    ...
    if (e.code === KEYBOARD_SHORTCUTS.DELETE) {
      !app.dragSelect.getCount() ||
        app.modal.open(ModalDelete, { items: app.dragSelect.getSelected() });
    }
